...
const vec3 = @import("./lib/math.zig").Vec3;
const mat4 = @import("./lib/math.zig").Mat4;
...
export fn frame() void {
    ...
    if (state.accumulator > 15.0){
        var point: foundation.POINT = .{ .x = 0, .y = 0};
        
        if(User32.GetCursorPos(&point) == 0) {
            std.debug.print("Erro ao obter posição do cursor", .{});
            std.debug.print("{}", .{error.GetCursorPosFailed});
        }
        
        std.debug.print("Pos: x={d}, y={d}\n", .{point.x, point.y});
        
                
        state.accumulator -= 15.0;
    }
